...
2)A network graph will be presented to demonstrate the connections between employees. Also, in order to find out the most frequent email connections, the number of email engagement will be treated as the weight of the graph, and by altering the weight people may clearly realize the level of connections of communities in GAStech company.
...
### News heatmap
A heatmap (or heat map) is efficient way to visualize hierarchical clustering. It allow us to simultaneously visualize clusters of news for different time and for different media. A color scheme is applied for the blocks to display ‘high’ and ‘low’ frequency values. ggplot2 is efficient in presenting this visualization.
